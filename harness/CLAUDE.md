...
| `showcase-director.py [--log f] [--wait s]` | `~/Zomboid/pzopt-showcase-state.json` from a `--flag showcase=horde --flag director=jev` run | Jev directs the showcase scene: one choice request per new state (run_to_pier, face_horde, light_fire_line, shoot, reload, hold), the answer written to `~/Zomboid/pzopt-showcase-cmd.txt`, every decision logged with probabilities and latency. Start it in the background before the run (it waits for a fresh state); it exits when the state is stale 15 s. Scene flags (`pzopt.Showcase`): `start=` (run start), `pier=x,y`, `horde_dir`, `horde`, `horde_gap`, `mags`, `gun`, `ambulance`, `lightbar`, `fire_ms`, `power=off`, `darkness=pitch`; with the Scene flags `weather=storm fog=heavy fog_tint=dark`. Pass `--option panCameraWhileAiming=false --option focusloss=false`. Take `cine-jev2` → `docs/media/hdr-horde-cinematic-jev.mp4` (`tools/hdr/cine-cut.sh`, local only: 502 MB) |
