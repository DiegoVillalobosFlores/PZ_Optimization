...
Cause and fix: `docs/findings-per-pixel-lighting-2026-09-25.md` (the render thread replays its last state at a chunk crossing;
a recycled pixelLight Frame carried the next frame's camera). Loop settings: `--prop devCapture=10,16,240,25` (16 s at a quarter
size, 21 for stock) catches ~7 bursts per run before the fix.

Stairs walk flicker rig (2026-09-25, the flip report "with every new lighting setting on the walls flicker going up to the
top floor and down to the basement"): `--mode bench --launcher direct --source-save Sandbox/2026-09-25_21-32-32 --flag
explore=stairs --flag director=jev --flag zombies=off --flag zoom=1 --flag route=S:1 --flag speed=0.008 --prop
devCapture=0,125,240,33,gray` with `harness/explore-director.py --machine flip` started before the job (one director per
run: it answers only states newer than itself). `pzopt.StairsWalk` walks the player on foot (movement keys, a path over every
level of the building with the staircases as edges) as Jev decides: look around on each level, up to the top floor, down to
the lowest (a basement is a BuildingDef of its own; every building overlapping it counts). ~40 s, Riverside police station
basement -> 1 -> basement. Then `stairs-flicker.py <run> [--control <run>] [--judge]`: blink numbers per phase of the walk
(between the `harness: stairs:` events); read the `solid` columns (3x3-eroded blink patches: a wall or light patch blinking),
not the raw px/bursts, which the 1-px edge shimmer of the moving camera fills in every build (stock has the most). The `,gray`
capture field writes one luma byte per pixel (a quarter of the bytes). `--prop devPplProbe=10` logs per frame how many squares'
native light / vision and packed lattice values changed and came back (A-B-A). Write-up: `docs/findings-wall-flicker-2026-09-25.md`.
...
| `region-flicker.py <run\|video> START END [--json f] [--heat png]` | a `devCapture` sequence (`<run>/capture`) or a lossless video | flicker.py's transient metric at the capture's own size, per 3 x 3 screen region (HUD and a disc round the player masked), per second for the top-left, the busiest cells, the worst frames, capture gaps (frames the writer dropped), and every burst frame (some region > 4 % blinking) with its region (`bursts` in the JSON) |
| `region-flicker-judge.py <test.json> <control.json> [--region top-left] [--out f]` | two region-flicker.py JSONs of the same walk, same capture size | Jev over numbers only, whole screen (a screen-wide blink lands in whichever region blinks most): `flicker_confirmed`, `stock_also`, `kind` (pzopt_flicker / shared_flicker / no_flicker / inconclusive). Validated 2026-09-25: the pixelLight replay bug vs stock `pzopt_flicker` 0.97, the fix `no_flicker` 0.83 |
| `seam-lines.py <run\|png>... [--control <run>] [--before <run>...] [--context "..."] [--out f]` | `--shot-at` captures (`shot-game.png`, `shot2-game.png`) of the same camera | thin dark seam lines along the iso grid: dark ridges on straight diagonal runs, `new_seam` = absent from the control's first shot (pixel-aligned, same size), `new_long_seam` = on runs of two squares and more (grid lines, not single tile / wall edges); with `--control` Jev answers `before_has_lines`, `test_has_lines`, `test_has_short_edges`, `kind` (fixed / still_lines / not_reproduced / inconclusive). 2026-09-25: pixelLight chunk-edge lines 1,700-2,000 new long px/MP -> 21-28 (control floor 0-6) |
...
| `ao-late-judge.py --test <run>... --control <run>... [--off <run>...] [--holes] [--out f]` | AO-on path drives (`drive-120-south --record`) with `ao_latency=` in `pzopt-bench.out` (ChunkAo.latency(), 2026-09-25) | Jev over numbers only: does a new chunk texture reach the screen before its first AO (grass / ground shading arriving late), is it fixed in `test`, did the frame tail (pzopt-overlay.out) or chunk arrival (drive-check.json, `--holes` = holes.py) regress; `verdict` fixed / fixed_with_cost / not_fixed / not_reproduced / inconclusive. Known: `aog-fix-*` vs `aog-old-*` → fixed 0.89 |
| `watermotion.py <run A> <run B> [--a2 <repeat A>] [--b2 <repeat B>] [--context "..."] [--mask-png /tmp/p]` | two `--record` runs of the still river-shore rig: `--mode bench --flag start=6450,5200 --flag find=shore --flag zombies=off --flag time_of_day=15 --flag weather=clear --flag route=S:1 --flag speed=0.1 --flag hold=20 --flag zoom=1.5 --route-seconds 30 --record --prop overlay=false` | does the river still flow? Water (median-colour blue-grey mask) vs land control over the last 14 s of the world on screen: frame-to-frame change at 15 fps, change over 1 s, temporal std shares, `spatial_detail` (ripple contrast, tells fainter from slower); Jev: flowing_like_reference / static_water / reduced_motion / more_motion. 2026-09-25 (runs `waterflow-*`): stock 0.173, optimized with upscaler off 0.165, fsr1 0.184, **DLSS 0.085** (same contrast, half the motion: its temporal blend damps the in-place animated ripples; fixed by `dlssWaterCurrent`). Pass `--prop upscaler=...` explicitly: the desktop's tab file sets `upscaler=dlss`. The capture is AV1 at `-q very_high`: more motion anywhere in the frame raises the encoder noise on static edges elsewhere, so a far-land change is not proof of a render change; check with the lossless `--shot-at` pair (`shot-game.png` / `shot2-game.png`). A queue `media` job |
...
| `contact/kernel_rig.py <out.png> [--hour H] [--box x0,y0,x1,y1,h]... [--steps N] [--time N]` | nothing (synthetic) | the chunk AO kernel's sun term offline (moderngl on EGL; `python -m venv /tmp/cs/venv && /tmp/cs/venv/bin/pip install moderngl numpy pillow`): the real GLSL from ChunkAo.java on a ray-cast ground + boxes depth, writes the term, times kernel + blur (2026-09-25). Its depths sit near 0.5; the game's near 0 (a sampler reading zeros hides there) |
| `contact/alt.py <run> [--skip S]` | console.txt (`devSunAlternate` line), pzopt-overlay.out | gpu_ms / frame time split by the capsule pass's on / off periods (`--prop devSunAlternate=<ms>`): a drift-free same-run A/B. Keep the period off any spin rate (2 s at 90 deg/s = on and off saw different halves) |
| `spritefilter/shotab.py <run>` | a `--shot-at` run with `--prop devSpriteFilterShotAb=true` (`shot-game.png` stock, `shot2-game.png` filtered) | changed share, gradient / Laplacian energy of both, crops of the most-changed tiles (2026-09-26, candidate A) |
| `spritefilter/modes.py <run> [--crop x,y,w,h]` | a `--shot-at` run with `--prop devSpriteFilterShotModes=a,b,c,d` + `--prop devCapture=4,20,6,100,gray` | one settled captured frame per mode (four slots of the hold clear of the harness screenshots, which stall ~0.9 s): detail (RMS Laplacian) and change vs stock, crop sheets |
| `spritefilter/alt.py <run>` | a run with `--prop devSpriteFilterAlternate=20 --prop devSpriteFilterCycle=stock,rgssa+skip,...` + `gpuSections=true` | composite GPU us per entry vs stock (bootstrap 95 %), and every other section attributed to the entry of the frame it ran in (bakes, mip passes) |
| `spritefilter/shimmer.py <run>...` | `devCapture` of a moving camera (e.g. `drive-120-south` + `--prop devCapture=14,1.5,60,100,gray`) | detail and motion-compensated Laplacian residual (crawl / shimmer) per run: phase-correlation shift per patch, Fourier-moved previous frame |
| `stitch-spritefilter.py <out.mp4>` | runs `sfv-{walk0.75,walk1.5,drive}-{stock,sharp}` with `--prop devCapture=<start>,<secs>,<fps>,100,crop=1601:380:1918:1400,ram` (drives with `--no-mangohud --prop overlay=false`) | `docs/media/sprite-filter-stock-vs-sharp.mp4` + poster: 1:1 panes and 2x nearest close-ups, results card; a queue media job. `devCapture` fields `crop=x:y:w:h` (1:1 region) and `ram` (frames held in native memory until written, exit waits for the writer: a full ZFS pool dropped seconds of 320 MB/s captures) |
...
| `stairs-flicker.py <run> [--control <run>] [--before <run>] [--crops N] [--judge] [--json f]` | a `devCapture` sequence of an `explore=stairs` walk + its console | blink px per frame per walk phase, solid-patch px (3x3-eroded mask) and patch frames (> 0.1 % of the world); `--crops` writes base / blink / after strips of the worst patches to `<run>/stairs-flicker/`; `--judge` asks Jev (numbers only) whether the walls flicker compared with the control |
| `stitch-stairs-flicker.py --flip <problem> <fix> [--desk <problem> <fix>] <out.mp4> [--jev f] [--mask ...]` | two flip `explore=stairs` walks with a colour `devCapture` (e.g. `devCapture=8,34,240,33`), optionally two desktop `--record` walks with a capture | `docs/media/stairs-wall-flicker-problem-vs-fix.mp4`: the walks side by side cut at the floor changes, 0.27x close-ups of the worst blinking walls, the desktop HDR ground-floor arrival; the recorder's start epoch from cross-correlating recording and capture luma (~0.9); AV1 10-bit PQ, SDR panes mapped to PQ at 203 nits |
| `explore-director.py [--machine flip] [--log f] [--wait s]` | `~/Zomboid/pzopt-explore-state.json` from a `--mode bench --flag explore=restaurant --flag director=jev` run (`explore_match=pizzawhirled` etc., `zombies=off`, `route=S:1 speed=0.004` as the time cap) | Jev walks the character (movement keys along a grid path, doors opened, never teleported) to the nearest matching building and through its ground-floor rooms: go_to_restaurant / next_room / look_around (a full turn, HDR dumps at the quarter turns of the first `explore_dump_rooms`) / face_north / hold / done; `--machine` runs Jev here and bridges the state / command files over one ssh connection (the laptops have no TypeSafe key). Without `director=jev` an autopilot plays the same order. Rig of the 2026-09-25 flip HDR report (`devHdrTraceMs` trace) |
| `trees/tree-director.py [--log f] [--wait s]` | `~/Zomboid/pzopt-trees-state.json` from a `--mode bench --flag explore=trees --flag director=jev` run (`trees_max`, `trees_radius`, `trees_watch`, `zombies=off`, `route=S:1 speed=0.004` as the time cap) | Jev walks the character (movement keys along a grid path, never teleported) to the nearest trees, one lap round each and a still watch: next_tree / circle_tree / watch / hold / done. `pzopt.TreeWalk` also writes every frame the on-screen trees' sprite rectangles and the camera to `pzopt-trees.out`. Desktop only (TypeSafe key). Rig of the 2026-09-25 tree lighting report |
| `trees/tree-metrics.py <run> [--json f] [--png dir]` | an `explore=trees` run with `--prop devCapture=start,secs,fps,scale` | per tree box: near-black share (black crowns), luma per storey band above the foot (the foot-to-top shading), and on still-camera frames the share of pixels jumping > 24/255 in the tree boxes and over the view (instability), burst frames |
| `trees/tree-judge.py TEST.json CONTROL.json [--context] [--out f]` | two tree-metrics.py JSONs | Jev: black_trees / unstable / lower_part_unshaded / verdict (bug_present, matches_control, inconclusive), numbers only |
| `trees/walk-ab.sh <label>:<extra args> ...` | the desktop queue | one Jev-directed tree walk per label (director in the background, queued run with a devCapture, then tree-metrics.py); `TW_START`, `TW_HOUR`, `TW_TREES` |
| `trees/tree_rig.py [--dump dir] [--out prefix] [--len-squares L] [--steps N] [--canopy s] [--sky s] [--crown r] [--no-trees] [--time N]` | a `--prop devAoDumpTree=N` run's `~/Zomboid/pzopt-chunkao/<k>/` (the first N computes with a tree in reach: source depths at full size, uniforms, the game's raw output) | replays that chunk AO / sun compute offline with the current GLSL of ChunkAo.java (moderngl venv of contact/kernel_rig.py): raw sun / AO / blurred term PNGs, the difference to the game's own output (0 = the rig matches), GPU time per compute. Queue it as a `cmd` job for timings (a game run on the GPU skews them) |
...
| `mac-update-e2e.sh [out]` | ON the Mac (ssh from a queue `cmd` job, after `--install opt` of the build) | the in-game updater end to end (2026-09-26): stages a newer Workshop copy of the installed build in the Mac's Steam library, boots to the menu with `devUpdateDrive=true` (the Lua presses Update now, then Restart game), waits for the restarted process to reach the menu and quit, checks build-info + every manifest hash, prints both consoles' `[pzopt] update/restart` lines, restores the install / properties / options and removes the copy. Refuses when a game runs, a flag file exists or a real item 3805285544 is present. First run: PASS, relaunch 60 ms after the old process ended, restarted menu 12.9 s after the press |
...
| `newcard.py` / `audio-card-gif.py` / `ao-card-gif.py` / `hdr-card-gif.py [--still png]` | the card's numbers + the recordings behind them (`audio-card-gif.py`: the `snd-i6-stress-*` pair) | the Workshop page's animated "New!" card (2026-09-24): the still card's style at 1920x1080, the change playing in the left half, intro + table on the right, footer under both; `--still` = the first frame without ffmpeg (layout check). `docs/workshop/images/26-clear-audio.gif`, `27-ambient-occlusion.gif` (AO off / on wipe over two pixel-aligned `--shot-at` captures); render as a queue `media` job |
| `darkness-card-gif.py [--still png --scene N --on]` | runs `card-{day,night,water}-{off,on}` (in-game `devCapture`, every visual Enhancements option off / on) | `docs/workshop/images/37-darkness-memory-grading.gif`, the release's New! card; also `stitch-darkness.py` (the side-by-side video from runs `cap-*`) and `menu-gifs.py darkness memory grade` (kind `pane`: the tab previews from the same captures) |
...
with the variables filled in: the test after editing `dashboards.py` (0 errors expected). `harness/grafana/ui-check.py [--base http://127.0.0.1:3000] [--all] [--sizes 1920x1080,1366x768,412x915]` is the visual test (2026-09-24): headless Chrome (`ui-check.mjs`, Node's WebSocket over the DevTools protocol) renders each dashboard and records per panel its text, No data / error markers, drawn canvas pixels, clipped text, overlaps and the daily chart's tooltips; Jev judges each panel against its title and description (ok / no_data / error / clipped / blank_chart / overlap / wrong_content) plus layout and hover; screenshots + report.json under `--out`. Chrome costs CPU: submit it as a queue `cmd` job, never beside a run. Findings under ~0.5 confidence are mostly Jev's doubt on legitimately empty or shortened cells.
...
- **Dashboards** (generated by `dashboards.py`, provisioned read-only; edit the generator): *PZ runs* (the home page; on top
  the public hero `HERO_SCENES`: stock vs optimized on the desktop, medians of the newest 10 runs per side, only exact scenes
  run both ways, no profiler, no pzopt key beyond instrument / overlay / uncappedFps / hdrAuto / frameCapFps; the scene with
  the most pairs is the headline: stock widget left, × in the middle, optimized widget right; under them "Each day's build on the <history on>": each day's build (the day's last release, before 09-21 its last commit)
  built in a detached worktree (`/tmp/pzopt-daily-<MMDD>`, the current queue scripts, 09-24's install.sh and 09-21's
  run-mac.sh copied in) and run through the queue from that worktree's own run.sh (`--install <worktree>` on the desktop,
  `--install opt` from the worktree on a laptop, each laptop under its own `PZQ_SESSION=<sid>-<machine>`) on every bench its
  harness can run, `dashboards.DAILY_BENCHES`: `daily` clear 120 km/h E:1200 drive (all days), `dstorm` the same in fog + storm
  and `dspin` the spin route south in fog + storm (09-20 on), `dlou` Louisville (09-21 on), `dhorde` riverside horde shooting
  (09-24 on, desktop only: its Jev director reads the local game's state file); labels `<bench>-<MMDD>-<n>`, stock
  `<bench>-stock-<n>`; boot and load of every run (its load trace) in a second chart under it, same hover notes; the variable `hmachine` ("history
  on") picks desktop / flip / dell / mac. Every optimized run forces the defaults the machines' tab files override
  (`--prop upscaler=off overlay=false hotsaveStaged=false chunkGridWidth=0 hdr=false inputLatch=true keyboardFresh=true
  treeBakeMaxChunksPerSec=0 enabled=true uncappedFps=true`, `--option vsync=false` on laptops); stock `--prop enabled=false
  --option frameRate=240 --option uncappedFPS=false`. The 09-22 / 09-23 builds carry the 2004681 road-follower fix (harness
  only). The hover lists what the day released:
  `release-days.json` notes, baked by `dashboards.py` into value mappings of invisible numeric fields (the tooltip drops
  text columns), one row per note; `excluded` there lists runs `grafana/daily-drives.py` marks BAD (console `harness: drive t=`
  telemetry: > 10 tiles off the line or a > 40 km/h drop). A new day: build, 2 runs per bench, add its notes, regenerate, `cloud.sh deploy`. Then the Steam Workshop row (subscribers, star rating, visitors,
  engagement = favorites / comments / awards / visitors subscribed %; table `workshop_stats`, a snapshot every 30 min by
  the follower through `workshop_stats.py`: keyless Web API for the counts, the item page for rating / comments / awards,
  NULL when Steam answers 429; never Steamworks, a process holding app 108600 looks like a running game to Steam), every
  matched scene in the table under that; then table with links,
  trends (one series per label up to 20 labels, else per machine · stock / optimized: 1,400 label series crashed the tab), "below the cap with hardware left over" = the objective's finding), *PZ run* (tails, fps per second,
