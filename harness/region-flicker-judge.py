...
A burst frame is one where more than 4 % of the region's world pixels blink (appear / vanish for 1-3 frames).
"""
import argparse
import json
import sys
...
def main():
    ...
    a = ap.parse_args()
    state = {
        "setup": "Two frame-exact captures (every presented frame read back in-game, 960x540) of the same scene: a copy of the "
    ...
    questions = {
        "flicker_confirmed": noul(f"Do the numbers confirm the report: does `test` show flicker in the {a.region} region, i.e. frames "
                                  "where a large part of that region blinks for one to three frames?"),
        "stock_also": noul(f"Does `control` (stock) show the same kind of {a.region} flicker at a comparable rate?"),
        "kind": choice({"question": f"What best describes `test`'s {a.region} region compared with `control`?",
                       ...
                       {"pzopt_flicker": "test has one-frame bursts in the region that control does not have: the mod causes the flicker",
