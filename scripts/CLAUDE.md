...
| `i18n.py [--check]` | UI translations (issue #21): extracts every translatable English text (Lua `PzoptT` / `T` calls, Java `I18n.text` calls, the options tab's tables: `<key>.label/.tip/.note.<v>`, `section.<first key>`, clips, axes) into `src/media/pzopt/translate/EN.json`, the translators' template; `--check` fails on a stale EN.json, one key with two English texts, or a translation whose `%N` placeholders differ, and lists each language's missing keys (they show English). build.sh runs both. |
