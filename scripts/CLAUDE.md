...
| `option-dates.py [-v]` | the date of the first release (`win-*` tag, by tag date) holding the commit that first declared each `pzopt.Config` key → `src/lua/client/pzopt/pzopt_optimizations_dates.lua`, the options tabs' "Sort by: Release date" and the preview's "Released" line; keys in no release yet are left out (shown as "New in this version": release.sh builds before it tags); build.sh runs it, without git history the committed file stays; `-v` lists the new keys. |
...
| `updater-bench.sh` | profiles the in-game updater against the real GitHub releases next to the pre-2026-09-26 updater (`tests/pzopt/UpdaterBench.java`; sections check / local / install / sweep / spans / http3 / workshop); downloads three release zips to `/tmp/pzopt-updater-bench` once. Network, light CPU: run through a queue `cmd` job. |
