...
public final class Config {
   ...
   public static final int DLSS_OUTPUT_PCT = integer("dlssOutputPct", 0); // dlss: DLSS writes this percentage of the screen size (its cost follows the output pixels) and EASU + RCAS finish the upscale; 0 = the screen size
   public static final String DLSS_OUTPUT_FILTER = string("dlssOutputFilter", "bicubic").trim().toLowerCase(java.util.Locale.ROOT); // dlss with dlssOutputPct: bicubic = the stock screen shader samples the smaller output directly (no extra pass), fsr1 = EASU + RCAS first
   public static final boolean DLSS_FLUSH_AFTER_WAIT = bool("dlssFlushAfterWait", true); // dlss: glFlush right after GL's wait on the DLSS-done semaphore (else the GPU idled ~0.2 ms until the next implicit flush)
